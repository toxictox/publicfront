...
export const getAllTransactions = async (page, count) => {
  const response = await axios.get(`${baseURL}/transaction?page=${page}&count=${count}`, {
    headers: getAuthHeaders(), // Используем токен в заголовках
  });
  return response.data;
};
...
export const acceptTransaction = async (transactionId) => {
  const response = await axios.post(`${baseURL}/transaction/${transactionId}/accept`, {}, {
    headers: getAuthHeaders(), // Используем токен в заголовках
  });
  return response.data;
};
...
export const getTransactionStatus = async (transactionId) => {
  const response = await axios.get(`${baseURL}/transaction/${transactionId}/status`, {
    headers: getAuthHeaders(), // Используем токен в заголовках
  });
  return response.data;
};
