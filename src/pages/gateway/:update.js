...
import UpdateGatewayForm from "@comp/gateway/UpdateGatewayForm";
import toast from "react-hot-toast";
import { BackButton } from "@comp/core/buttons";

const GatewayIdUpdate = () => {
  ...
  return (
    <>
      <Helmet>
        <title>{t("Gateway Item Update")}</title>
      </Helmet>
      <Box
        sx={{
        ...
        <Container maxWidth={settings.compact ? "xl" : false}>
          ...
          <Box sx={{ minWidth: 700 }}>
            <Card sx={{ mt: 2 }}>
              ...
              <Divider />
              {dataList !== null ? (
                <UpdateGatewayForm data={dataList} callback={handleSubmit} />
              ) : null}
            </Card>
          </Box>
        </Container>
...
export default GatewayIdUpdate;
